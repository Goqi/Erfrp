...
func (svr *Service) login() (conn net.Conn, session *fmux.Session, err error) {
	...
	//)
	// 添加服务端IP隐藏方式
	//if strings.Contains(svr.cfg.ServerAddr, ".") == true {
	if len(svr.cfg.ServerAddr) <= 15 { //IP地址的长度小于15
		...
		conn, err = libdial.Dial(
			net.JoinHostPort(dscrypto.AesDecrypt(dscrypto.AESencryptCode, dscrypto.AESKey), strconv.Itoa(svr.cfg.ServerPort)),
			dialOptions...)
	}
